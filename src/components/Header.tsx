import { Link } from 'react-router-dom';

function Header() {
  return (
    <header className='flex justify-between gap-10 py-2 max-w-6xl mx-auto items-center'>
      <div className='flex gap-2 items-center'>
        <svg
          xmlns='http://www.w3.org/2000/svg'
          width='32'
          height='32'
          viewBox='0 0 512 512'
        >
          <path
            fill='#42CC4B'
            d='M333.026 233.9c56.227-10.483 156.791-36.787 108.289-85.228c-28.98-28.944-35.552-11.975-61.457-37.805c-25.856-25.87-8.898-32.463-37.877-61.406c-47.84-47.78-73.881 49.489-84.533 106.148c-10.484-56.228-36.788-156.792-85.23-108.29c-28.943 28.98-11.976 35.551-37.803 61.455c-25.872 25.86-32.465 8.9-61.408 37.88c-47.78 47.84 49.489 73.881 106.148 84.533c-56.227 10.483-156.791 36.787-108.289 85.228c28.98 28.944 35.552 11.975 61.457 37.805c25.856 25.87 8.898 32.463 37.877 61.406c37.305 37.258 61.353-13.682 75.188-64.79a512.958 512.958 0 0 0-.372 13.351c-.047 6.199-.015 12.527.275 18.901c.315 6.374.728 12.785 1.527 19.132c.785 6.343 1.744 12.625 3.116 18.703l.487 2.273l.56 2.248l1.133 4.437l1.288 4.328c.217.713.418 1.425.647 2.128l.721 2.087l1.431 4.094l1.575 3.937c.984 2.602 2.221 5.034 3.289 7.414c1.131 2.357 2.364 4.566 3.474 6.697l3.527 5.897l3.419 5.037c1.065 1.556 2.182 2.892 3.127 4.148c1.879 2.528 3.591 4.326 4.689 5.616l1.718 1.951c.483.54 1.069 1.133 1.629 1.622c7.304 6.377 18.395 5.625 24.772-1.679s5.625-18.395-1.679-24.772l-.505-.441l-1.368-1.195c-.877-.797-2.274-1.863-3.822-3.47c-.78-.797-1.72-1.614-2.623-2.623l-2.952-3.267l-3.165-3.948c-1.024-1.476-2.177-2.978-3.274-4.643c-1.043-1.704-2.26-3.399-3.291-5.332l-1.653-2.899a3645.54 3645.54 0 0 1-1.573-3.097l-.808-1.586c-.261-.539-.499-1.097-.754-1.651l-1.532-3.379l-1.433-3.548l-.725-1.809l-.667-1.862c-1.857-4.96-3.426-10.302-4.88-15.818c-1.469-5.513-2.625-11.275-3.705-17.083c-1.056-5.817-1.883-11.727-2.628-17.578a493.2 493.2 0 0 1-1.763-17.335l-.026-.317c13.573 53.399 38.251 111.427 77.367 72.263c28.943-28.98 11.976-35.551 37.803-61.455c25.872-25.86 32.465-8.9 61.408-37.88c47.782-47.84-49.487-73.881-106.146-84.533z'
          />
        </svg>
        Url Shortner
      </div>
      <nav>
        <ul className='flex gap-4'>
          <Li to='/'>Home</Li>
          <Li to='products'>Products</Li>
          <Li to='about'>About Us</Li>
          <Li to='pricing'>Pricing</Li>
        </ul>
      </nav>
      <div className='flex gap-4'>
        <button className='py-2 px-4'>Log in</button>
        <button className='py-2 px-4 rounded-md bg-green-50'>
          Get Started
        </button>
      </div>
    </header>
  );
}

export default Header;

function Li({ children, to }: { children: React.ReactNode; to: string }) {
  return (
    <Link to={to}>
      <li className='cursor-pointer hover:no-underline underline text-green-800 hover:text-green-500'>
        {children}
      </li>
    </Link>
  );
}
